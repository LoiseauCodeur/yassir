            ...
            _fn: {
                ...
                set_text: function (obj, val) {
                    ...
                    if (this._get_settings().core.html_titles) {
                        ...
                        obj.html(escapeHtml(val)).prepend(tmp);
                ...
                _parse_json: function (js, obj, is_callback) {
                    ...
                    if ($.isArray(js)) {
                        ...
                        $.each(js.data, function (i, m) {
                            ...
                            if (typeof m == "string") {
                                tmp.attr('href', '#');
                                $('<span>').addClass('jstree-node-title').html(m).appendTo(tmp);
                            } else {
                                if (!m.attr) {
                                    m.attr = {};
                                }
                                ...
                                tmp.attr(m.attr);
                                $('<span>').addClass('jstree-node-title').html(m.title).appendTo(tmp);
                                if (m.language) {
                                    tmp.addClass(m.language);
                                }
                            }
...
//*/
    ...
    (function ($) {
        ...
        $(function () {
            var css_string = '' +
                '#vakata-dragged ins { display:block; text-decoration:none; width:24px; height:24px; margin:0 0 0 0; padding:0; position:absolute; top:0px; left:0px;' +
                ' -moz-border-radius:4px; border-radius:4px; -webkit-border-radius:4px; ' +
                '} ' +
                '#vakata-dragged .jstree-ok { background:green; } ' +
                '#vakata-dragged .jstree-invalid { background:red; } ' +
                '#jstree-marker { padding:0; margin:0; font-size:12px; overflow:hidden; height:12px; width:8px; position:absolute; top:-30px; z-index:10001; background-repeat:no-repeat; display:none; background-color:transparent; text-shadow:1px 1px 1px white; color:black; line-height:10px; } ' +
                '#jstree-marker-line { padding:0; margin:-2px 0px 0px -2px; line-height:0%; font-size:1px; overflow:hidden; height:1px; width:100px; position:absolute; top:-30px; z-index:10000; background-repeat:no-repeat; display:none; background-color:#0091B5; ' +
                ' cursor:pointer; border:1px solid #ffffff; border-left:0;' +
                '}' +
                '';
            $.vakata.css.add_sheet({str: css_string, title: "jstree"});
